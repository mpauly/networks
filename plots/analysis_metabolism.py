...
# %% [markdown]
# ## Separate various trajectories - part I
# In this section we split the trajectories according to if they have two maxima and which of the two is higher.
#
...
# %% [markdown]
# ## Separate various trajectories - part II
# In this section we split out all trajectories that have an early minimum, i.e. that drop below $1$ before $\sigma = 10$.

# %%
data["hasearlymin"] = False


# %%
for sn in startnodes:
    data.loc[data["start_node"] == sn, "hasearlymin"] = (
        (data["start_node"] == sn) & (data["sigma"] < 10) & (data["dim"] < 1.0)
    ).any()


# %%
data["hasearlymin"].value_counts()


# %%
fig, ax = plt.subplots(2, figsize=(8, 12))
for ttype in [False, True]:
    mean_per_sigma = data[data["hasearlymin"] == ttype].groupby("sigma").mean()
    data_plot = np.array(
        list(
            data[data["hasearlymin"] == ttype]
            .groupby("start_node")
            .apply(pd.DataFrame.to_numpy)
        )
    )
    data_plot = data_plot[:, :, 1:3]
    line_segments = LineCollection(data_plot, alpha=0.2)
    ax[int(ttype)].add_collection(line_segments)
    ax[int(ttype)].plot(mean_per_sigma.index, mean_per_sigma["dim"], c="tab:red")
