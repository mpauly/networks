...
for walk_type, col in enumerate(["tab:green", "tab:blue"]):
    data_plot = np.array(
        list(
            data[data["tratype"] == walk_type + 1]
            .groupby("start_node")
            .apply(pd.DataFrame.to_numpy)
        )
    )
    data_plot = data_plot[:, :, 1:3]
    line_segments = LineCollection(data_plot, alpha=0.2, color=col)
    ax1.add_collection(line_segments)

mean_per_sigma = data.groupby("sigma").mean()
ax1.plot(mean_per_sigma.index, mean_per_sigma["dim"], c="tab:orange")
